...
import type { CategoryType, FunnelStage } from '../types';
...
const SortableFunnelHeader: React.FC<SortableFunnelHeaderProps> = ({
  ...
  const parentLabel = allFunnels.find((entry) => entry.id === funnel.parentFunnelId)?.name ?? 'None';
      ...
      <div className="space-y-3 px-4 py-3 text-left">
        ...
        <div className="space-y-1">
          ...
          {canEdit ? (
            ...
            <ReadonlyField value={formatNumber(funnel.targets[activeCategory])} />
          )}
        </div>
      </div>
    </th>
            ...
            <div className="rounded-2xl border border-slate-100 bg-slate-50/60 p-3">
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  <span>Conv from {conversion.parentName ?? '-'}:</span>
                  <span
                    className={cn(
                      'rounded-md px-1.5 py-0.5 text-[11px] font-bold normal-case tracking-normal',
                      conversion.rate === null ? 'bg-slate-200 text-slate-500' : 'bg-blue-100 text-blue-700',
                    )}
                  >
              ...
              <div className="space-y-2">
                <div className="space-y-1">
                  ...
                  {canEdit ? (
                    ...
                    <ReadonlyField value={formatNumber(cellData.visits)} />
                  )}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {canEdit ? (
                    ...
                    <ReadonlyField value={`฿${formatNumber(cellData.revenue)}`} />
                  )}
                </div>
              </div>
            </div>
...
export const MonitoringTableAuth: React.FC = () => {
  ...
  const getConversionLabel = (funnelId: string) => {
    const parentFunnel = getParentFunnel(funnelId);
    return parentFunnel ? `Conv from ${parentFunnel.name}` : 'Conv from -';
  };
                  ...
                  <div className="relative flex h-full items-center justify-between px-4 py-4">
                    <div>
                      ...
                      <span className="mt-2 block text-sm font-semibold text-slate-700">
                        {isEditor ? 'Drag to reorder rows' : 'Read-only view'}
                      </span>
                    </div>
                    {isEditor ? (
                      <div
                  ...
                  <SortableContext items={funnelIds} strategy={horizontalListSortingStrategy}>
                    {funnels.map((funnel) => (
                      <SortableFunnelHeader
                        ...
                        onDeleteRequest={(funnelId, name) => setPendingDelete({ type: 'funnel', id: funnelId, name })}
                      />
                    ))}
                  </SortableContext>
                </DndContext>
                ...
                <tbody>
                  {channels.map((channel) => (
                    <SortableChannelRow
                      ...
                      onDeleteRequest={(channelId, name) => setPendingDelete({ type: 'channel', id: channelId, name })}
